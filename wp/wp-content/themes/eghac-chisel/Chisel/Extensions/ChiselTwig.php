...
class ChiselTwig extends Twig {
	...
	/**
	 * Finds and replaces a tag with a tag with arrow inside
	 *
	 * @return string
	 */
	public function replaceLinkarrow( $content, $leave_regular = false ) {
		if (!$leave_regular) {
			preg_match("'<a (.*?)</a>'si", $content, $match);
			if ($match) {
				$link = '<p class="link-wrapper"><a class="link" ' . $match[1] . ' <i class="icon-arrow-right"><span class="path1"></span><span class="path2"></span></i></a></p>';
	...
	/**
	 * Makes iframe non-html for load on click
	 *
	 * @return string
	 */
	public function videoClickLoad( $video ) {
		preg_match('/video">(.*?)<\/div>/', $video, $match);
		if ($match[1]) {
			$content = $match[1];
			...
			if ($src && $src[1]) {
				if (strpos($src[1], '?') > -1) {
					$video_src = $src[1] . '&autoplay=1';
				}
			}
			if ($video_src) {
				$content_new = str_replace( $src[1], $video_src, $content );
			}
			return str_replace( $content, htmlentities( $content_new ), $video );
		} 
		return $video;
	}
